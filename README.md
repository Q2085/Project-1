...
![正確預測＞145mm 事件的氣象參數，以及對應 Expected Gradient 詮釋的合成平均](/pic/image%203.png)
...
右：850 百帕緯向風對於預測 >145mm 的重要性貢獻](/pic/image%208.png)
...
右：所有正確預測 >145mm 事件之850百帕天氣圖合成平均。顏色代表可降水量](/pic/image%209.png)
...
- 本專案展示了，XAI方法不僅能讓我們深入理解模型的判斷依據，更可以協助找尋模型存在的缺陷，替未來模型的改善提供明確的方向
- XAI分析表明，模型對強降雨事件的預測依據，與傳統預報方法頗為吻合，例如兩者皆重視低層噴流與高層輻散的重要性
- 分析模型嚴重誤報的樣本後，發現這些樣本的背景環境條件，與其它強降雨事件確實有所差異。透過XAI方法，可以驗證並量化這樣的相異環境，如何對預測結果帶來影響。
- 基於分析結果，專案提出建議如下：① 將不同環境條件下的降雨類型納入訓練，或者 ② 探索能夠反映降水空間型態差異的訓練方法
